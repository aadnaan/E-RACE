...
export default class Chat extends React.Component {
  ...
  async componentDidMount() {
    await this.setState({
      ...
      Status: this.props.navigation.state.params.CalledBy
    });
  ...
  getMessages() {
    ...
    firestore
      .collection("Chat")
      .where("ListerID", "==", this.state.ListerID)
      .where("RenterID", "==", this.state.RenterID)
      .onSnapshot(snapshot => {
        snapshot.docs.forEach(doc => {
          const exists = doc.data() !== null;
          if (exists) {
            data = doc.data();
            if (this.state.Status === "Lister") {
              this.setState({
