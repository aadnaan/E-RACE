...
import { View, StyleSheet, Dimensions ,Text,FlatList,TouchableHighlight} from 'react-native';
...
addToRentedFlatList2(RentedTrips_feed,data,userdetail){
  var that=this
  firestore.collection('Cars').doc(data.ListingID).get().then(async doc=>{
...
addToRentedFlatList1(RentedTrips_feed,data){
  var that=this;
  firestore.collection('Users').doc(data.Lister).get().then(doc=>{
      const exists=(doc.data()!==null)
      if(exists){
          userdetail=doc.data()
...
addToListedFlatList1(ListedTrips_feed,data){
  var that=this;
  firestore.collection('Users').doc(data.Renter).get().then(doc=>{
      const exists=(doc.data()!==null)
      if(exists){
          userdetail=doc.data()
...
loadNew2=()=>{
  ...
  var that=this;
  firestore.collection('CompletedTrips').get().then((snapshot)=>{
        ...
        return( 
          <View style={{flex:1,margin:7}}>
            ...
            renderItem={({item,index})=>(
                <TouchableHighlight onPress={()=>this.BookedSelectedHandler(item)} >
                    <BookedTrips 
      ...
      case 'second':
        return(
          <View style={{flex:1,margin:7}}>
            ...
            keyExtractor={(item,index)=>index.toString()}
            renderItem={({item,index})=>(
                <TouchableHighlight onPress={()=>this.ListedSelectedHandler(item)}>
                    <ListedTrips 
                      Thumbnail={item.Thumbnail}
